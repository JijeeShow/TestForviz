...
function Inmonth({ date, roomid, data }) {
  const [alldays, setAlldays] = useState();

  useEffect(() => {
    getBookingsInMonth(date);
  }, []);
  ...
  const getBookingsInMonth = (datein) => {
    const start = startOfMonth(datein);
    const end = endOfMonth(datein);
    const daysOfMonth = eachDayOfInterval({
      start,
      end,
    });
    setAlldays(daysOfMonth);
  };
